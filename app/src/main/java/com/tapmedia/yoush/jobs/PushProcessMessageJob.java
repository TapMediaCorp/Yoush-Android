...
import com.tapmedia.yoush.conversation.background.BackgroundJobReceiver;
import com.tapmedia.yoush.conversation.job.MessageJob;
import com.tapmedia.yoush.conversation.pin.PinJobReceiver;
...
import com.tapmedia.yoush.groups.GroupV1MessageProcessor;
...
import com.tapmedia.yoush.linkpreview.Link;
import com.tapmedia.yoush.linkpreview.LinkPreview;
import com.tapmedia.yoush.linkpreview.LinkPreviewUtil;
...
import com.tapmedia.yoush.sms.IncomingEncryptedMessage;
import com.tapmedia.yoush.sms.IncomingEndSessionMessage;
      ...
      throws VerificationFailedException, IOException, InvalidGroupStateException, GroupChangeBusyException
  {
    try {
      ...
      if (content.getDataMessage().isPresent()) {
        ...
        if (PinJobReceiver.handleReceivedMessage(content, message, groupId)) return;
        if (BackgroundJobReceiver.handleReceivedMessage(content, message, groupId)) return;
        if (isGv2Message) {
        ...
        if      (isInvalidMessage(message))             handleInvalidMessage(content.getSender(), content.getSenderDevice(), groupId, content.getTimestamp(), smsMessageId);
          ...
          if (message.getBody().isPresent() && isJSONValid(message.getBody().get())) {
            ...
            if (messageType.equals("groupCall") || messageType.equals("call")) {
              ...
              String callState = json.getString("state");

              if (callState.equals("dialing")) {
                SignalServiceDataMessage groupMessage = SignalServiceDataMessage.newBuilder()
                        .withBody("Có người đang gọi đến") // TODO: ĐA NGÔN NGỮ - DƯ THỪA, KHÔNG CÓ Ý NGHĨA GÌ
                        .build();

                Recipient recipientGroup = getMessageDestination(content, message);
                Recipient recipient = getMessageDestination(content, groupMessage);

                String groupName = recipientGroup.getName(context);
                if (messageType.equals("call")) {
                  groupName = null;
                }
                String senderName = recipient.getProfileName().getGivenName();

                Intent intent = new Intent(context, GroupCallBeginService.class);
                RemotePeer remotePeer = new RemotePeer(recipientGroup.getId());

                // activity
                String roomName = json.getString("room");
                String audioOnly = json.getString("audioOnly");
                String subject = json.getString("subject");
                intent.setAction(GroupCallBeginService.ACTION_RECEIVE_OFFER)
                        .putExtra(GroupCallBeginService.ROOM_NAME, roomName)
                        .putExtra(GroupCallBeginService.GROUP_NAME, groupName)
                        .putExtra(GroupCallBeginService.SENDER_NAME, senderName)
                        .putExtra(GroupCallBeginService.AUDIO_ONLY, audioOnly)
                        .putExtra(GroupCallBeginService.MESSAGE_TYPE, messageType)
                        .putExtra(GroupCallBeginService.SUBJECT, subject)
                        .putExtra(GroupCallBeginService.EXTRA_REMOTE_PEER, remotePeer);
                ...
                Intent intent = new Intent(context, GroupCallBeginService.class);
                intent.setAction(GroupCallBeginService.ACTION_CHANGE_STATE) //TODO: ????
                        .putExtra(GroupCallBeginService.EXTRA_REMOTE_PEER, remotePeer);

                context.startService(intent);
              } else if (callState.equals("busy")) {
                JitsiService obj = new JitsiService();
                obj.hangUp(context, true);
              }
              return;
            }
          }
          handleGroupV1Message(content, message, smsMessageId, groupId.get().requireV1());
        }
        ...
        else if (message.getBody().isPresent())         { // ANDROID

          if (isJSONValid(message.getBody().get())) {
            JSONObject json = new JSONObject(message.getBody().get());
            String messageType = json.getString("messageType");

            if (messageType.equals("groupCall") || messageType.equals("call")) {

              String roomNameCheck = json.getString("room");

              OkHttpClient client = new OkHttpClient();

              Request request = new Request.Builder().url("/room-size?room=" + roomNameCheck).build();
              ...
              client.newCall(request).enqueue(new Callback() {
                ...
                public void onResponse(Call call, Response response) throws IOException {
                  try {
                    ...
                    Boolean allowAlertCall = false;

                    Long timestampCall = message.getTimestamp();
                    Long duration = Long.valueOf((((1 * 60) + 0) * 1000));
                    Long timestampAfter = timestampCall + duration;

                    Long now = System.currentTimeMillis();

                    if (timestampAfter > now) {
                      allowAlertCall = true;
                    } else {
                      if (response.code() == 200) {
                        allowAlertCall = true;
                      } else {
                        allowAlertCall = false;
                      }
                    }

                    if (allowAlertCall && callState.equals("dialing")) {

                        SignalServiceDataMessage groupMessage = SignalServiceDataMessage.newBuilder()
                                .withBody("Có người đang gọi đến") // TODO: ĐA NGÔN NGỮ
                                .build();
                        ...
                        intent.setAction(GroupCallBeginService.ACTION_RECEIVE_OFFER)
                                ...
                                .putExtra(GroupCallBeginService.AUDIO_ONLY,audioOnly)
                                .putExtra(GroupCallBeginService.MESSAGE_TYPE,messageType)
                                .putExtra(GroupCallBeginService.CALL_ID,callId)
                                .putExtra(GroupCallBeginService.SUBJECT,subject)
                                .putExtra(GroupCallBeginService.EXTRA_REMOTE_PEER,                remotePeer);
                  ...
                  } catch (JSONException | BadGroupIdException e) {
                    e.printStackTrace();
                  }
                }
              });
            ...
            handleTextMessage(content, message, smsMessageId, groupId);
          }
        }
