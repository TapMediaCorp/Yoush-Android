package com.tapmedia.yoush.search;
...
import com.tapmedia.yoush.database.SearchDatabase;
...
public class SearchRepository {
  ...
  public void query(@NonNull String query, @NonNull Callback<SearchResult> callback) {
    ...
    serialExecutor.execute(() -> {
      ...
      Future<List<ThreadRecord>> conversations = parallelExecutor.submit(() -> {
        List<ThreadRecord> list = queryConversations(cleanQuery);
        List<ThreadRecord> hiddenList = queryHideConversations(cleanQuery);
        if (!hiddenList.isEmpty()) {
          list.addAll(hiddenList);
        }
        return list;
      });
      Future<List<ThreadRecord>> hideConversations = parallelExecutor.submit(() -> Collections.EMPTY_LIST);
      Future<List<MessageResult>> messages = parallelExecutor.submit(() -> queryMessages(cleanQuery));

      try {
        long startTime = System.currentTimeMillis();
        SearchResult result = new SearchResult(cleanQuery, contacts.get(), conversations.get(), hideConversations.get(), messages.get());
  ...
  public void hiddenResult(String query, Callback<SearchResult> callback) {
    if (TextUtils.isEmpty(query)) {
      callback.onResult(SearchResult.EMPTY);
      return;
    }
    ...
    serialExecutor.execute(() -> {
      ...
      Future<List<ThreadRecord>> hideConversations = parallelExecutor.submit(() -> ActionData.getHiddenThreadRecords());
      Future<List<MessageResult>> messages = parallelExecutor.submit(() -> queryMessages(cleanQuery));
  ...
  private @NonNull List<ThreadRecord> queryHideConversations(@NonNull String query) {
    String pinCode = SignalStore.pinCodeValues().getPinCode();
    if(pinCode.equals(query)){
      try (Cursor cursor = threadDatabase.getConversationListByHidden(true)) {
        return readToList(cursor, new ThreadModelBuilder(threadDatabase));
      }
    } else {
      List<String>      numbers      = contactAccessor.getNumbersForThreadSearchFilter(context, query);
      List<RecipientId> recipientIds = Stream.of(numbers).map(number -> Recipient.external(context, number)).map(Recipient::getId).toList();
      try (Cursor cursor = threadDatabase.getHideConversationStatusList(recipientIds, "1")) {
        return readToList(cursor, new ThreadModelBuilder(threadDatabase));
      }
    }
  }
