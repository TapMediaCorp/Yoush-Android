...
import com.tapmedia.yoush.mediasend.AvatarSelectionActivity;
import com.tapmedia.yoush.mediasend.AvatarSelectionBottomSheetDialogFragment;
...
import com.tapmedia.yoush.util.StringUtil;
...
import static com.tapmedia.yoush.profiles.edit.EditProfileActivity.DISPLAY_USERNAME;
import static com.tapmedia.yoush.profiles.edit.EditProfileActivity.EXCLUDE_SYSTEM;
import static com.tapmedia.yoush.profiles.edit.EditProfileActivity.GROUP_ID;
import static com.tapmedia.yoush.profiles.edit.EditProfileActivity.NEXT_BUTTON_TEXT;
import static com.tapmedia.yoush.profiles.edit.EditProfileActivity.NEXT_INTENT;
import static com.tapmedia.yoush.profiles.edit.EditProfileActivity.SHOW_TOOLBAR;
...
public class EditProfileFragment extends LoggingFragment {
  ...
  private void initializeResources(@NonNull View view, boolean isEditingGroup) {
    ...
    this.givenName .addTextChangedListener(new AfterTextChanged(s -> {
      ...
      if(!string.isEmpty()){
        ...
        if(!s.toString().equals(string)){
          this.givenName.setText(string);
          int textLength = this.givenName.getText().toString().length();
          this.givenName.setSelection(textLength);
        }
        trimInPlace(s, isEditingGroup);
        viewModel.setGivenName(string);
      }
    ...
    if (isEditingGroup) {
      ...
      view.<ImageView>findViewById(R.id.avatar_placeholder).setImageResource(R.drawable.ic_group_outline_40_new);
    } else {
      ...
      this.familyName.addTextChangedListener(new AfterTextChanged(s -> {
        ...
        if(!string.isEmpty()){
          ...
          if(!s.toString().equals(string)){
            ...
            int textLength = this.familyName.getText().toString().length();
            this.familyName.setSelection(textLength);
          }
          trimInPlace(s, false);
          viewModel.setFamilyName(string);
        }
      }));
