...
import com.tapmedia.yoush.attachments.MmsNotificationAttachment;
...
import com.tapmedia.yoush.database.documents.IdentityKeyMismatchList;
import com.tapmedia.yoush.database.documents.NetworkFailure;
import com.tapmedia.yoush.database.documents.NetworkFailureList;
...
import com.tapmedia.yoush.database.model.NotificationMmsMessageRecord;
import com.tapmedia.yoush.database.model.Quote;
...
import com.tapmedia.yoush.revealable.ViewOnceExpirationInfo;
...
import static com.tapmedia.yoush.contactshare.Contact.Avatar;
...
public class MmsDatabase extends MessagingDatabase {
      ...
      while (cursor.moveToNext()) {
        if (Types.isOutgoingMessageType(cursor.getLong(cursor.getColumnIndexOrThrow(MESSAGE_BOX)))) {
          ...
          if (ourRecipientId.equals(theirRecipientId) || Recipient.resolved(theirRecipientId).isGroup()) {
            ...
            DatabaseFactory.getThreadDatabase(context).update(threadId, false, false);
  ...
  private void updateMailboxBitmask(long id, long maskOff, long maskOn, Optional<Long> threadId) {
    ...
    if (threadId.isPresent()) {
      DatabaseFactory.getThreadDatabase(context).update(threadId.get(), false, false);
    }
  }
...
//  }
  ...
  void trimEntriesForExpiredMessages() {
    ...
    try (Cursor cursor = database.query(ThreadDatabase.TABLE_NAME, new String[] { ThreadDatabase.ID }, ThreadDatabase.EXPIRES_IN + " > 0", null, null, null, null)) {
      while (cursor != null && cursor.moveToNext()) {
        DatabaseFactory.getThreadDatabase(context).update(cursor.getLong(0), false, false);
      }
    }
  }
  ...
  private Pair<Long, Long> updateMessageBodyAndType(long messageId, String body, long maskOff, long maskOn) {
    ...
    DatabaseFactory.getThreadDatabase(context).update(threadId, true, false);
    notifyConversationListeners(threadId);
  ...
  public void markIncomingNotificationReceived(long threadId) {
    ...
    if (com.tapmedia.yoush.util.Util.isDefaultSmsProvider(context)) {
      DatabaseFactory.getThreadDatabase(context).incrementUnread(threadId, 1);
    }
      ...
      throws MmsException
  {
    ...
    ContentValues contentValues = new ContentValues();
    DevLogger.d("insertMessageOutbox");
      ...
      throws MmsException
  {
    ...
    try {
      ...
      DatabaseFactory.getThreadDatabase(context).update(contentValues.getAsLong(THREAD_ID), true, false);
    }
  }
  ...
  public boolean delete(long messageId) {
    ...
    boolean threadDeleted = DatabaseFactory.getThreadDatabase(context).update(threadId, false, false);
    notifyConversationListeners(threadId);
  ...
  public class Reader implements Closeable {
    ...
    private NotificationMmsMessageRecord getNotificationMmsMessageRecord(Cursor cursor) {
      ...
      if (!TextUtils.isEmpty(contentLocation))
        contentLocationBytes = com.tapmedia.yoush.util.Util.toIsoBytes(contentLocation);

      if (!TextUtils.isEmpty(transactionId))
        transactionIdBytes = com.tapmedia.yoush.util.Util.toIsoBytes(transactionId);
