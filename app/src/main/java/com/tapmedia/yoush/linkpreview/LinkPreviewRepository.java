package com.tapmedia.yoush.linkpreview;
...
import com.tapmedia.yoush.conversation.preview.PreviewJob;
...
import com.tapmedia.yoush.net.CallRequestController;
import com.tapmedia.yoush.net.CompositeRequestController;
import com.tapmedia.yoush.net.ContentProxySafetyInterceptor;
import com.tapmedia.yoush.net.ContentProxySelector;
import com.tapmedia.yoush.net.RequestController;
...
public class LinkPreviewRepository {
  ...
  RequestController getLinkPreview(@NonNull Context context, @NonNull String url, @NonNull Callback<Optional<LinkPreview>> callback) {
    ...
    if (StickerUrl.isValidShareLink(url)) {
      ...
      metadataController = fetchMetadata(url, metadata -> {
        if (PreviewJob.handleLinkPreview(callback,url)) {
          return;
        }
        if (metadata.isEmpty()) {
          callback.onComplete(Optional.absent());
  ...
  public static RequestController fetchThumbnail(@NonNull Context context, @NonNull String imageUrl, @NonNull Callback<Optional<Attachment>> callback) {
