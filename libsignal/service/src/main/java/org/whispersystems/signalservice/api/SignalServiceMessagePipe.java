...
public class SignalServiceMessagePipe {
  ...
  public Future<SendMessageResponse> send(OutgoingPushMessageList list, Optional<UnidentifiedAccess> unidentifiedAccess, boolean isCall, String subject, String phoneNumber, boolean isSilent, String groupName, String callerName, boolean audioOnly) throws IOException {
    ...
    OutgoingPushMessageList newList = null;

    List<OutgoingPushMessage> messages = new LinkedList<>();

    int silent = isSilent ? 1 : 0;

    for (int i=0; i < list.getMessages().size(); i++) {
      OutgoingPushMessage old = list.getMessages().get(i);
      OutgoingPushMessage newOut = old;
      newOut.setGroupId(subject);
      newOut.setIsVoip(isCall);
      newOut.setCallerPhoneNumber(phoneNumber);
      newOut.setSilent(silent);
      newOut.setAudioOnly(audioOnly);

      newOut.setCallerName(callerName);

      if (groupName.equals("null")) {
        newOut.setGroupName(callerName);
      } else {
        newOut.setGroupName(groupName);
      }

      if (callerName.equals("null")) {
        newOut.setCallerName(phoneNumber);
        if (groupName.equals("null")) {
          newOut.setGroupName(phoneNumber);
        }
      }
    ...
    newList = new OutgoingPushMessageList(list.getDestination(), list.getTimestamp(), messages, list.isOnline());

    WebSocketRequestMessage requestMessage = WebSocketRequestMessage.newBuilder()
                                                                    .setId(new SecureRandom().nextLong())
                                                                    .setVerb("PUT")
                                                                    .setPath(String.format("/v1/messages/%s", newList.getDestination()))
                                                                    .addAllHeaders(headers)
                                                                    .setBody(ByteString.copyFrom(JsonUtil.toJson(newList).getBytes()))
                                                                    .build();
