...
public class SignalServiceMessageSender {
          ...
          throws IOException, UntrustedIdentityException
  {
    ...
    sendMessage(recipient, getTargetUnidentifiedAccess(unidentifiedAccess), message.getWhen(), content, false, null, false, "", "",false, "", "", false);
  }
          ...
          throws IOException, UntrustedIdentityException
  {
    ...
    sendMessage(recipient, getTargetUnidentifiedAccess(unidentifiedAccess), message.getTimestamp(), content, true, null, false, "", "",false, "", "", false);
  }
  ...
  {
    byte[] content = createTypingContent(message);
    sendMessage(recipients, getTargetUnidentifiedAccess(unidentifiedAccess), message.getTimestamp(), content, true, cancelationSignal, false, "", "",true, "", "", false);
  }
          ...
          throws IOException, UntrustedIdentityException
  {
    ...
    sendMessage(recipient, getTargetUnidentifiedAccess(unidentifiedAccess), System.currentTimeMillis(), content, false, null, false, "", "",false, "","", false);
  }
          ...
          throws UntrustedIdentityException, IOException
  {
    ...
    if (message.getBody().isPresent() && isJSONValid(message.getBody().get())) {
      JSONObject json = new JSONObject(message.getBody().get());
      ...
      if (json.getString("messageType") != null) {
        ...
        if (messageType.equals("groupCall") || messageType.equals("call")) {
          ...
          audioOnly = Boolean.parseBoolean(json.getString("audioOnly"));

            if (state.equals("dialing")) {
              subject = json.getString("room");
              phoneNumber = json.getJSONObject("caller").getString("phoneNumber");
              isCall = true;
              groupName = json.getString("subject");
              callerName = json.getJSONObject("caller").getString("callerName");
            }

          if (state.equals("endCall")) {
            silent = 1;
          }
        }
      }
    ...
    SendMessageResult result    = sendMessage(recipient, getTargetUnidentifiedAccess(unidentifiedAccess), timestamp, content, false, null, isCall,subject,phoneNumber,silent == 1 ? true : false, groupName, callerName, audioOnly);
    ...
    if (result.getSuccess() != null && result.getSuccess().isNeedsSync()) {
      ...
      sendMessage(localAddress, Optional.<UnidentifiedAccess>absent(), timestamp, syncMessage, false, null, false, "", "",false,"", "", false);
    }
          ...
          throws IOException, UntrustedIdentityException
  {
    ...
    List<SendMessageResult> results            = sendMessage(recipients, getTargetUnidentifiedAccess(unidentifiedAccess), timestamp, content, false, null, isCall,subject,phoneNumber,silent == 1 ? true : false, groupName, callerName, audioOnly);
    ...
    if (needsSyncInResults || isMultiDevice.get()) {
      ...
      String subject1 = "";
      String phoneNumber1 = "";
      boolean isCall1 = false;
      int silent1 = 0;
      String groupName1 = "";
      String callerName1 = "";
      boolean audioOnly1 = false;
      ...
      if (message.getBody().isPresent() && isJSONValid(message.getBody().get())) {
        ...
        if (json.getString("messageType") != null) {
          ...
          if (messageType.equals("groupCall") || messageType.equals("call")) {
            String state = json.getString("state");
            audioOnly1 = Boolean.parseBoolean("audioOnly");

              if (state.equals("dialing")) {
                subject1 = json.getString("room");
                phoneNumber1 = json.getJSONObject("caller").getString("phoneNumber");
                isCall1 = true;
                groupName1 = json.getString("subject");
                callerName1 = json.getJSONObject("caller").getString("callerName");
              }
      ...
      sendMessage(localAddress, Optional.<UnidentifiedAccess>absent(), timestamp, syncMessage, false, null, isCall1,subject1,phoneNumber1,silent1 == 1 ? true : false, groupName1, callerName1, audioOnly1);
          ...
          throws IOException, UntrustedIdentityException
  {
    ...
    sendMessage(localAddress, Optional.<UnidentifiedAccess>absent(), timestamp, content, false, null, false, "", "",false,"","", false);
  }
          ...
          throws IOException, UntrustedIdentityException
  {
    ...
    SendMessageResult result = sendMessage(message.getDestination(), getTargetUnidentifiedAccess(unidentifiedAccess), message.getTimestamp(), content, false, null, false, "", "",false, "", "", false);

    if (result.getSuccess().isNeedsSync()) {
      byte[] syncMessage = createMultiDeviceVerifiedContent(message, nullMessage.toByteArray());
      sendMessage(localAddress, Optional.<UnidentifiedAccess>absent(), message.getTimestamp(), syncMessage, false, null, false, "", "",false, "", "", false);
    }
  }
      ...
      throws IOException
  {
    ...
    while (recipientIterator.hasNext()) {
      ...
      futureResults.add(executor.submit(() -> sendMessage(recipient, access, timestamp, content, online, cancelationSignal, isCall, subject, phoneNumber, isSilent, groupName, callerName, audioOnly)));
    }
    ...
    for (int i = 0; i < RETRY_COUNT; i++) {
      ...
      try {
        ...
        if (pipe.isPresent() && !unidentifiedAccess.isPresent()) {
          try {
            SendMessageResponse response = pipe.get().send(messages, Optional.absent(), isCall, subject, phoneNumber, isSilent, groupName, callerName, audioOnly).get(10, TimeUnit.SECONDS);
          ...
          try {
            SendMessageResponse response = unidentifiedPipe.get().send(messages, unidentifiedAccess, isCall, subject, phoneNumber, isSilent, groupName, callerName, audioOnly).get(10, TimeUnit.SECONDS);
